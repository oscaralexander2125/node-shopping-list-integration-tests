...
describe('Recipes', function() {
  ...
  it('it should list recipes on GET', function() {
  ...
  it('it should add recipe on POST', function() {
    const newItem= {
      name: 'coke',
      ingredients: [
        'sugar', 'water'
  ...
  it('it should update recipe on PUT', function() {
    const updateDatas= {
      name: 'sprite',
      ingredients: ['less sugar', 'water']
    };
    return chai.request(app)
    .get('/recipes')
    .then(function(res) {
      updateDatas.id=res.body[0].id;
      return chai.request(app)
      .put(`/recipes/${updateDatas.id}`)
      .send(updateDatas)
    })
    .then(function(res) {
      expect(res).to.have.status(204);
      /*expect(res).to.be.json;
      expect(res.body).to.be.a('object');
      expect(res.body).to.deep.equal(updateDatas);*/ // why wont this work here but it does in the shoppin list example
    });
  });
  it('should delete items on DELETE', function() {
    return chai.request(app)
